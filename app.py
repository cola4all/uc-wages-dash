...
@app.callback(
    ...
    Output('loading-name-search','children'),
...
def search_names(n_clicks, search_name: str):
    ...
    df_names_match = df_names[df_names[DataSchema.NAME].str.contains(search_name.casefold().strip())]
    unique_names_match = list(set(df_names_match[DataSchema.NAME]))
    
    # handle if too many matches
    ...
    for name in unique_names_match:
        ...
        years_available_str = ', '.join(years_available)
...
def update_plots(initial_wage: list[float], jobs: list[str], names = list[str], years = list[str]):
    ...
    for job_abbr in unique_job_abbr:
        dff_year_range.loc[dff_year_range[DataSchema.JOB_ABBREVIATED]==job_abbr,DataSchema.CUMADJUSTMENT] = dff_year_range.loc[dff_year_range[DataSchema.JOB_ABBREVIATED]==job_abbr,DataSchema.ADJUSTMENT].cumprod()
